  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm divide-y divide-slate-100">
        {loading && <div className="p-4 text-sm text-slate-400">Načítavam...</div>}
        {!loading && announcements.length === 0 && (
          <div className="p-4 text-sm text-slate-400">Zatiaľ žiadne oznámenia.</div>
        )}
        {announcements.map((a) => (
          <div key={a.id} className="p-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">{format(new Date(a.created_at), 'd.M.yyyy HH:mm')}</p>
            </div>
